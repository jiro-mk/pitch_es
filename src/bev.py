...
undistortImage = np.zeros_like(rawImage)
K = np.array([[627.235434, 0, 654.957574], [0, 630.482585, 494.346943], [0, 0, 1]])
D = np.array([-0.210146, 0.030563, 0.001172, -0.001306])
...
topImage = np.zeros_like(rawImage)
topImageGray = np.zeros_like(grayImage)
...
for y in range(topImage.shape[0]):
    for x in range(topImage.shape[1]):
        ...
        topImage[newY, newX, 0] = undistortImage[y, x, 0]
        topImage[newY, newX, 1] = undistortImage[y, x, 1]
        topImage[newY, newX, 2] = undistortImage[y, x, 2]
...
cv2.imshow("Top Image Gray", topImageGray)
