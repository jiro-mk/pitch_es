...
% This example shows how to construct a monocular camera sensor simulation capable 
% of lane boundary and vehicle detections. The sensor will report these detections 
% in vehicle coordinate system. In this example, you will learn about the coordinate 
% system used by the Automated Driving System Toolbox?, and computer vision techniques 
% involved in the design of a sample monocular camera sensor.
%% Overview
% Vehicles that contain ADAS features or are designed to be fully autonomous 
% rely on multiple sensors. These sensors can include sonar, radar, lidar and 
% cameras. This example illustrates some of the concepts involved in the design 
% of a monocular camera system. Such a sensor can accomplish many tasks, including:
% 
...
% used to issue lane departure warnings, collision warnings, or to design a lane 
% keep assist control system. In conjunction with other sensors, it can also be 
% used to implement an emergency braking system and other safety-critical features.
% 
% The example implements a subset of features found on a fully developed 
% monocular camera system. It detects lane boundaries and backs of vehicles, and 
% reports their locations in the vehicle coordinate system.
%% Define Camera Configuration
% Knowing the camera's intrinsic and extrinsic calibration parameters is critical 
% to accurate conversion between pixel and vehicle coordinates.
% 
% Start by defining the camera's intrinsic parameters. The parameters below 
% were determined earlier using a camera calibration procedure that used a checkerboard 
% calibration pattern. You can use the <matlab:helpview(fullfile(docroot,'toolbox','vision','vision.map'),'visionCameraCalibrator'); 
% cameraCalibrator> app to obtain them for your camera.
...
% Temporary
R = camParams.RotationMatrices(:,:,1);
TranslationVectors = camParams.TranslationVectors(1,:);

%% 
% Note that the lens distortion coefficients were ignored, because there 
% is little distortion in the data. The parameters are stored in a |<matlab:doc('cameraIntrinsics'); 
...
%% 
% Next, define the camera orientation with respect to the vehicle's chassis. 
% You will use this information to establish camera extrinsics that define the 
% position of the 3-D camera coordinate system with respect to the vehicle coordinate 
% system.

% height, Pitch, Yaw, Roll, SensorLocation 推定する
height = TranslationVectors(3)/1000;    % mounting height in meters from the ground
%http://planning.cs.uiuc.edu/node102.html
%http://planning.cs.uiuc.edu/node103.html
yaw = -atan2(R(2,1),R(1,1));
pitch = -atan2(-R(3,1),sqrt(R(3,2).^2+R(3,3).^2))*180/pi;
roll = -atan2(R(3,2),R(3,3));
%pitch  = 10;        % pitch of the camera in degrees
%% 
% The above quantities can be derived from the rotation and translation 
% matrices returned by the |<matlab:doc('extrinsics'); extrinsics>| function. 
% Pitch specifies the tilt of the camera from the horizontal position. For the 
% camera used in this example, the roll and yaw of the sensor are both zero. The 
% entire configuration defining the intrinsics and extrinsics is stored in the 
% |<matlab:doc('monoCamera'); monoCamera>| object.

%sensor = monoCamera(camIntrinsics, height, 'Pitch', pitch);
sensor = monoCamera(camIntrinsics, height, 'Pitch', pitch, 'Yaw', yaw, 'Roll', roll);
%% 
% Note that the |monoCamera| object sets up a very specific  <matlab:helpview(fullfile(docroot,'driving','helptargets.map'),'drivingCoordinates'); 
% vehicle coordinate system>, where the X-axis points forward from the vehicle 
% and the Y-axis points to the left of the vehicle.
% 
% 
% 
% The origin of the coordinate system is on the ground, directly below the 
% camera center defined by the camera's focal point. Additionally, |<matlab:doc('monoCamera'); 
% monoCamera>| provides |<matlab:doc('monoCamera/imageToVehicle'); imageToVehicle>| 
% and  |<matlab:doc('monoCamera/vehicleToImage'); vehicleToImage>| methods for 
% converting between image and vehicle coordinate systems.
% 
% Note: The conversion between the coordinate systems assumes a flat road. 
% It is based on establishing a homography matrix that maps locations on the imaging 
% plane to locations on the road surface. Nonflat roads introduce errors in distance 
% computations, especially at locations that are far from the vehicle.
%% Load a Frame of Video
% Before processing the entire video, process a single video frame to illustrate 
% the concepts involved in the design of a monocular camera sensor.
% 
% Start by creating a |<matlab:doc('VideoReader'); VideoReader>| object that 
% opens a video file. To be memory efficient, |VideoReader| loads one video frame 
% at a time.
...
sampleSubDirNames = folderNames;
sampleSubDirName = sampleSubDirNames(1);
imds = imageDatastore(fullfile(sampleDirPath,sampleSubDirName));

% imageDirctory = 'caltech_cordova1.avi';
% videoReader = VideoReader(videoName);

%% 
% Read an interesting frame that contains lane markers and a vehicle.
imageFilePaths = fullfile(sampleDirPath, sampleSubDirName);
%opts = detectImportOptions(imageFilePaths);

csvSampleDirPath = outputDirectoryPathInSlProj('csv_Samples');
csvFileName = sprintf('ID%08d.csv', eval(sampleSubDirName));
warning off
metaData = readtable(fullfile(csvSampleDirPath, csvFileName));
warning on

csvFilePath = fullfile(csvSampleDirPath, csvFileName);
[~,~,name] = xlsread(csvFilePath,'A1:M1');
name = regexprep(name,'\(|\)|\/','_');
name = regexprep(name,'\.','');


opts = detectImportOptions(csvFilePath);
T = readtable(csvFilePath, opts);
T.Properties.VariableNames = name;

%timeStamp = 0.06667;                   % time from the beginning of the video
timeStamp = 10/size(metaData,1);

frame = readimage(imds,2); % read frame at timeStamp seconds
imshow(frame) % display frame
%% 
% Note: This example ignores lens distortion. If you were concerned about 
% errors in distance measurements introduced by the lens distortion, at this point 
% you would use the |<matlab:doc('undistortImage'); undistortImage>| function 
% to remove the lens distortion.
%% Create Bird's-Eye-View Image
% There are many ways to segment and detect lane markers. One approach involves 
% the use of a bird's-eye-view image transform. Although it incurs computational 
% cost, this transform offers one major advantage. The lane markers in the bird's-eye 
% view are of uniform thickness, thus simplifying the segmentation process. The 
% lane markers belonging to the same lane also become parallel, thus making further 
% analysis easier.
% 
% Given the camera setup, the |<matlab:doc('birdsEyeView'); birdsEyeView>| 
% object transforms the original image to the bird's-eye-view. It lets you specify 
% the area that you want transformed using vehicle coordinates. Note that the 
% vehicle coordinate units were established by the |<matlab:doc('monoCamera'); 
% monoCamera>| object, when the camera mounting height was specified in meters. 
% For example, if the height was specified in millimeters, the rest of the simulation 
...
% 
% Note that you can complete the latter processing steps without use of the 
% bird's-eye view, as long as you can locate lane boundary candidate pixels in 
% vehicle coordinates.
%% Find Lane Markers in Vehicle Coordinates
% Having the bird's-eye-view image, you can now use the |<matlab:doc('segmentLaneMarkerRidge'); 
% segmentLaneMarkerRidge>| function to separate lane marker candidate pixels from 
% the road surface. This technique was chosen for its simplicity and relative 
% effectiveness. Alternative segmentation techniques exist including semantic 
% segmentation (deep learning) and steerable filters. You can substitute these 
% techniques below to obtain a binary mask needed for the next stage.
% 
% Most input parameters to the functions below are specified in world units, 
% for example, the lane marker width fed into |segmentLaneMarkerRidge|. The use 
% of world units allows you to easily try new sensors, even when the input image 
% size changes. This is very important to making the design more robust and flexible 
% with respect to changing camera hardware and handling varying standards across 
% many countries.
...
% are anchored to the camera sensor. This example uses a parabolic lane boundary 
% model, ax^2 + bx + c, to represent the lane makers. Other representations, such 
% as a third-degree polynomial or splines, are possible. Conversion to vehicle 
% coordinates is necessary, otherwise lane marker curvature cannot be properly 
% represented by a parabola while it is affected by a perspective distortion.
% 
...
%% 
% Since the segmented points contain many outliers that are not part of 
% the actual lane markers, use the robust curve fitting algorithm based on random 
% sample consensus (RANSAC).
% 
% Return the boundaries and their parabola parameters (a, b, c) in an array 
% of |<matlab:doc('parabolicLaneBoundary'); parabolicLaneBoundary>| objects, |boundaries|.
...
% |validateBoundaryFcn|. This example function is listed at the end of this example. 
% Using this additional input lets you reject some curves based on the values 
% of the a, b, c parameters. It can also be used to take advantage of temporal 
% information over a series of frames by constraining future a, b, c values based 
% on previous video frames.
%% Determine Boundaries of the Ego Lane
% Some of the curves found in the previous step might still be invalid. For 
% example, when a curve is fit into crosswalk markers. Use additional heuristics 
% to reject many such curves.
...
%% 
% Remove additional boundaries based on the strength metric computed by 
% the |<matlab:doc('findParabolicLaneBoundaries'); findParabolicLaneBoundaries>| 
% function. Set a lane strength threshold based on ROI and image size.
...
%% 
% The heuristics to classify lane marker type as single/double, solid/dashed 
% are included in a helper function listed at the bottom of this example. Knowing 
% the lane marker type is critical for steering the vehicle automatically. For 
% example, passing another vehicle across double solid lane markers is prohibited.
...
% Detecting and tracking vehicles is critical in front collision warning (FCW) 
% and automated emergency braking (AEB) systems.
% 
% Load an aggregate channel features (ACF) detector that is pretrained to 
% detect the front and rear of vehicles. A detector like this can handle scenarios 
% where issuing a collision warning is important. It is not sufficient, for example, 
% for detecting a vehicle traveling across a road in front of the ego vehicle.
...
% function to specialize the generic ACF detector to take into account the geometry 
% of the typical automotive application. By passing in this camera configuration, 
% this new detector searches only for vehicles along the road's surface, because 
% there is no point searching for vehicles high above the vanishing point. This 
% saves computational time and reduces the number of false positives.

monoDetector = configureDetectorMonoCamera(detector, sensor, vehicleWidth);

[bboxes, scores] = detect(monoDetector, frame);
%% 
% Because this example shows how to process only a single frame for demonstration 
% purposes, you cannot apply tracking on top of the raw detections. The addition 
% of tracking makes the results of returning vehicle locations more robust, because 
% even when the vehicle is partly occluded, the tracker continues to return the 
% vehicle's location. For more information, see the <docid:driving_examples.mw_8ac84c1c-03e4-48f4-9e2a-b37b33c42a74 
% Track Multiple Vehicles Using a Camera> example.
% 
% Next, convert vehicle detections to vehicle coordinates. The |computeVehicleLocations| 
% function, included at the end of this example, calculates the location of a 
% vehicle in vehicle coordinates given a bounding box returned by a detection 
% algorithm in image coordinates. It returns the center location of the bottom 
% of the bounding box in vehicle coordinates. Because we are using a monocular 
% camera sensor and a simple homography, only distances along the surface of the 
% road can be computed accurately. Computation of an arbitrary location in 3-D 
% space requires use of stereo camera or another sensor capable of triangulation.
...
% Now that you have an idea about the inner workings of the individual steps, 
% let's put them together and apply them to a video sequence where we can also 
% take advantage of temporal information.
% 
% Rewind the video to the beginning, and then process the video. The code 
% below is shortened because all the key parameters were defined in the previous 
% steps. Here, the parameters are used without further explanation.
return
...
% The |<matlab:edit('helperMonoSensor'); helperMonoSensor>| class assembles 
% the setup and all the necessary steps to simulate the monocular camera sensor 
% into a complete package that can be applied to any video. Since most parameters 
% used by the sensor design are based on world units, the design is robust to 
% changes in camera parameters, including the image size. Note that the code inside 
% the |helperMonoSensor| class is different from the loop in the previous section, 
% which was used to illustrate basic concepts.
% 
% Besides providing a new video, you must supply a camera configuration corresponding 
% to that video. The process is shown here. Try it on your own videos.
...
% *visualizeSensorResults* displays core information and intermediate results 
% from the monocular camera sensor simulation.
...
%% 
% *computeVehicleLocations* calculates the location of a vehicle in vehicle 
% coordinates, given a bounding box returned by a detection algorithm in image 
% coordinates. It returns the center location of the bottom of the bounding box 
% in vehicle coordinates. Because a monocular camera sensor and a simple homography 
% are used, only distances along the surface of the road can be computed. Computation 
% of an arbitrary location in 3-D space requires use of a stereo camera or another 
% sensor capable of triangulation.
...
%% 
% *insertVehicleDetections* inserts bounding boxes and displays [x,y] locations 
% corresponding to returned vehicle detections.
...
%% 
% *validateBoundaryFcn* rejects some of the lane boundary curves computed 
...
%% 
% *classifyLaneTypes* determines lane marker types as |solid|, |dashed|, 
% etc.
